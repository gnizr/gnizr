...
var ajaxSuggestions = {
	// Settings
	elmIdToPresentResultsIn : "ajax-suggestions-results",
	elmIdResultsContainer : "ajax-suggestions-container",
	...
	init : function (){
		...
		if(this.xmlHttp){
			...
			if(this.hideRsultsOnDocumentClick){
	...
	makeSuggestionCall : function (value, url){
		...
		if(!exists){
			...
			this.currentURL = url;
			//alert('curr url ' + this.currentURL);
	...
	presentResult : function (){
		MochiKit.DOM.removeElementClass(this.elmToPresentResultsIn,'invisible');
		this.elmToPresentResultsIn.innerHTML = this.suggestions[this.resultIndex][1];
		//alert('content: ' + this.suggestions[this.resultIndex][1]);
		/*
	...
	clearResults : function (justClear){
		if(this.elmResultsContainer && this.elmToPresentResultsIn){
			...
			MochiKit.DOM.addElementClass(this.elmToPresentResultsIn,'invisible');
	...
	insertValueIntoField : function (evt){
		...
		var value = MochiKit.DOM.getNodeAttribute(elm,'title');	
		/*
